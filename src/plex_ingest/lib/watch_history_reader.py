...
_COLUMNS = "imdb_id, title, year, genres, imdb_rating, summary, last_viewed_at"
...
def _row_to_watch_history(imdb_id: str, row: tuple[Any, ...]) -> WatchHistoryRow:
    title, year, genres, imdb_rating, summary, last_viewed_at = row[1:]
...
def fetch_watch_history_movie(
    conn: DuckDBPyConnection, imdb_id: str
) -> WatchHistoryRow:
    """The `stg_watch_history` row for `imdb_id`. Raises if it's missing — every asset
    that reads this table by partition key needs the same "must exist" guarantee."""
    row = conn.execute(
        f"SELECT {_COLUMNS} FROM stg_watch_history WHERE imdb_id = ?",  # noqa: S608 — _COLUMNS is a module constant, not user input
        [imdb_id],
    ).fetchone()
    if row is None:
        msg = f"No stg_watch_history row for imdb_id={imdb_id!r}"
        raise ValueError(msg)
    return _row_to_watch_history(imdb_id, row)


def fetch_all_watch_history(conn: DuckDBPyConnection) -> dict[str, WatchHistoryRow]:
    """Every `stg_watch_history` row, keyed by imdb_id — unbounded, since
    ...
    window actually gets enforced, by filtering the imdb_ids it builds points for —
