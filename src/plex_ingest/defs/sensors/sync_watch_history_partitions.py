...
from plex_ingest.defs.partitions import watch_history_tmdb_id_partitions
...
# Matches sync_tmdb_id_partitions's interval (see docs/pipeline-design.md's
...
def _missing_embeddings(tmdb_id: str) -> bool:
    return not WATCH_HISTORY_EMBEDDINGS_IO_MANAGER.path_for(tmdb_id).exists()


def compute_new_partition_ids(
    desired_ids: set[str], registered_ids: set[str]
) -> set[str]:
    """Add-only, deliberately -- unlike sync_tmdb_id_partitions's
...
def sync_watch_history_partitions(
    ...
    """Keeps the `watch_history_tmdb_id` dynamic partition set in sync with
    `stg_watch_history`, and is the sole trigger for `watch_history_embeddings`'s
    first materialization (mirroring `sync_tmdb_id_partitions`'s reasoning for
    ...
    **Add-only**, unlike `sync_tmdb_id_partitions`: a tmdb_id already partitioned
    ...
    guarantees a given tmdb_id is only ever embedded once, matching the requirement
    driving this whole pipeline's design -- see docs/pipeline-design.md's
    "Watch-history diversity-recommender pipeline".

    Unlike `sync_tmdb_id_partitions`, this sensor never requests
    ...
    `sync_tmdb_id_partitions`'s docstring and docs/pipeline-design.md's "Known gaps",
    ...
    doesn't exist at all. There is no add-only equivalent of `sync_tmdb_id_partitions`'s
    ...
    with duckdb.get_connection() as conn:
        current_ids = {
            row[0]
            for row in conn.execute("SELECT tmdb_id FROM stg_watch_history").fetchall()
        }

    registered_ids = set(
        watch_history_tmdb_id_partitions.get_partition_keys(
    ...
    if new_ids:
        dynamic_partitions_requests.append(
            watch_history_tmdb_id_partitions.build_add_request(sorted(new_ids))
        )
        context.log.info(f"watch_history_tmdb_id partitions: +{len(new_ids)}")
    ...
    for tmdb_id in sorted(current_ids | registered_ids):
        if not _missing_embeddings(tmdb_id):
            continue
        signature = f"{tmdb_id}:embeddings"
