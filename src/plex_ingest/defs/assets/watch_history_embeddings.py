...
@dg.asset(
    partitions_def=watch_history_tmdb_id_partitions,
...
def watch_history_embeddings(
    ...
    `embeddings` (media_items), there's exactly one point per tmdb_id here — no
    ...
    `sync_tmdb_id_partitions`'s docstring for the full mechanism). `deps=` on
    ...
    with duckdb.get_connection() as conn:
        row = fetch_watch_history_movie(conn, tmdb_id)
    ...
    context.log.info(f"{tmdb_id}: embedded watch-history document")
