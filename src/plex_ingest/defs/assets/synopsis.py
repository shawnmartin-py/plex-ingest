...
def synopsis(
    ...
    tmdb_id; the imdb_id the scraper structurally needs (IMDb pages are only
    addressable by tt-id) is looked up from the movie's stg_movies row. Carries no
    automation_condition: `sync_tmdb_id_partitions` is the sole
    ...
    text = scraper.fetch_synopsis(movie.imdb_id, movie.title, movie.year)
    found = "found" if text else "no"
    context.log.info(
        f"{movie.title} ({tmdb_id}): {found} synopsis ({len(text or '')} chars)"
    )
    return text
