...
def embeddings(
    ...
    movie — matches vector-store-contract.md's "up to 4 points per tmdb_id" (1 synopsis
    ...
    semantics". `sync_tmdb_id_partitions` also backfills this directly
    ...
    if not synopsis:
        msg = f"{tmdb_id} has no synopsis — enrichment should not have run without one"
    ...
    context.log.info(f"{tmdb_id}: embedded {len(result)} document(s)")
    return result
